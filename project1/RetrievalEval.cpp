...
/*
 #Name:Vivek Ponnala
 #Date: 2/4/2021
 #Title: Retrieval Evaluation Algorithms
*/
...
//weight(qt,dt,Q,D) = term_freq(dt)*Weight(qt)
...
// compute the weight of a matched term
//weight(qt,dt,Q,D) = term_freq(dt)*log(Numdocs/Numdocs(term))*Weight(qt)
		     ...
		     Index *ind)
{
  ...
  double result = double(ind->docCount()/ind->docCount(termID));
  double res= double((std::log(result)));
  return docTermFreq*res*qryTermWeight; 
}


// compute the weight of a matched term
//weight(qt,dt,Q,D) = log(term_freq(dt)+1)*log(Numdocs/Numdocs(term))*Weight(qt)
		     ...
		     Index *ind)
{
  ...
  double result = double(ind->docCount()/ind->docCount(termID));
  double res= double(std::log(result));
  return log(docTermFreq+1)*res*qryTermWeight;
 
}

// compute the weight of a matched term
//weight(qt,dt,Q,D) = dt/(dt+0.5+1.5*len(D)/avgLen(D))*log(NumDocs-numDocs(termt)+0.5/numdocs(term)+0.5)*(8+weight/7+weight)
		     ...
		     Index *ind)
{
  ...
  double doc = double(ind->docLength(docID)/ind->docLengthAvg());
  double term = double(docTermFreq/(docTermFreq+0.5+1.5*doc));
  double result = double((ind->docCount() - ind->docCount(termID) + 0.5)/(ind->docCount(termID) + 0.5));
  double res = double(std::log(result));
  double weights = (double(8+qryTermWeight))/(double(7+qryTermWeight));
  return term * res * weights;
}
